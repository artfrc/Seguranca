...
from .interfaces.user_register import IUserRegister
...
class UserRegister(IUserRegister):
    ...
    def registry(self, username: str, password: str) -> Dict:
        hashed_password = self.__cretae_hashed_password(password)
        self.__registry_new_user(username, hashed_password)
        return self.__format_response(username)

    def __cretae_hashed_password(self, password: str) -> str:
